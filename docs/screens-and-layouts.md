...
A `Show real layout` checkbox overlays colored markers directly on the real photo at each modeled control's true position. Modeled so far: XDJ-XZ's transport cluster (PLAY/PAUSE, CUE, SYNC, jog wheel, tempo fader) and DDJ-XP2's pad cluster (the 16-pad grid, the 4 PAD MODE buttons, and the SLIDE FX bank — EFFECT 1/2/3, FX LEVEL, TOUCH STRIP HOLD). It's disabled with a tooltip for any controller with no modeled geometry yet. This overlay is decorative reference only, same as the rest of this tab — it doesn't reflect the loaded mapping.
