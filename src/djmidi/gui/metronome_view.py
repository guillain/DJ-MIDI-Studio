"""Independent Metronome dock: a loop-oriented MIDI session player driven by
the current Controller Setup session (play once or repeat at a Hz frequency).

Previously merged into MIDI Routing's "Controller Setup playback" panel
(commit d1e1f1d); pulled back out into its own View-menu dock so it does not
compete for space with routing/Clock configuration.
"""
