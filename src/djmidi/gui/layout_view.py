...
class ControllerLayoutView(QWidget):
    ...
    def _draw_control_shape(
        ...
        if visual_kind in ("pad", "button"):
            ...
            shape.setPen(_CONTROL_PEN)
            center_x, center_y = left + 16, top + 16
            angle = _knob_angle_rad(self._values.get(key, _MIDI_DEFAULT))
            tip_x = center_x + 11 * math.sin(angle)
            tip_y = center_y - 11 * math.cos(angle)
            ...
            thumb_top = _fader_thumb_top(self._values.get(key, _MIDI_DEFAULT), top, 32, 8)
