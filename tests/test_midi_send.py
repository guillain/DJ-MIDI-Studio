...
from seratomidiconf import midi_send


def test_cli_list_ports_returns_zero(capsys):
    with patch("sys.argv", ["seratomidiconf-send-midi", "--list-ports"]), patch(
        "seratomidiconf.midi_send.list_output_ports", return_value=["Port A", "Port B"]
    ):
        rc = midi_send.main()
...
def test_cli_sends_message_when_all_args_present():
    ...
    ), patch("seratomidiconf.midi_send.send_midi_message") as mock_send:
        rc = midi_send.main()
    assert rc == 0
...
def test_cli_returns_error_code_on_send_failure(capsys):
    ...
    ), patch("seratomidiconf.midi_send.send_midi_message", side_effect=ValueError("boom")):
        rc = midi_send.main()
...
def test_cli_missing_required_args_exits_with_parser_error():
    with patch("sys.argv", ["seratomidiconf-send-midi", "--port", "Port A"]):
        try:
            midi_send.main()
